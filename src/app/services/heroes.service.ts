...
import { Response, Movie, Heroe } from '../models/api-models';
...
*/
  movies$: BehaviorSubject<Heroe[]> = new BehaviorSubject(this.initialHeroe);  
  ...
  getPeliculas(): any {
    return this.http.get(`${environment.baseUrl}peliculas`);
    //return this.movies$.asObservable();
  }

  filterHeroes(searchText: string) {  
    const filteredMovies = this.movies$.value.filter((movie) => movie.nombre.toLowerCase().includes(searchText.toLowerCase()) );  
    this.movies$.next(filteredMovies);
  } 

  resetHeroes() {
    this.movies$.next(this.initialHeroe);
  }
}
